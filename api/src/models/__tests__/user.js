...
test("image is passed to the client", () => {
  const imgUrl = "http://example.com/avatar.png"
  const userWithImage = generateUser({
    image: imgUrl
  })
  const result = userWithImage.toProfileJSONFor()
  expect(result).toHaveProperty("image", imgUrl)
})
